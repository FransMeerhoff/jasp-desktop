...
OpenSaveWidget::OpenSaveWidget(QWidget *parent) : QWidget(parent)
{
	...
	// OSF Old
	_bsOSFOld = new BackstageOSFOld(_tabWidget);
	...
	_tabWidget->addTab(_bsOSFOld, "OSF Old");
	...
	connect(_bsOSFOld, SIGNAL(dataSetIORequest(FileEvent *)), this, SLOT(dataSetIORequestHandler(FileEvent *)));
...
void OpenSaveWidget::setOnlineDataManager(OnlineDataManager *odm)
{
	...
	_bsOSFOld->setOnlineDataManager(odm);
...
void OpenSaveWidget::setSaveMode(FileEvent::FileMode mode)
{
	...
	_bsOSFOld->setMode(_mode);
	_bsOSFOld->setCurrentFileName(getDefaultOutFileName());
